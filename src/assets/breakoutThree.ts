import { BreakoutType } from "../types/round-type";

const breakoutThree: BreakoutType = {
  title: "Parabula bilang isang Misteryosong Panitikan",
  description:
    "Ang Augustus Code ay isang code na kinakailangan na i-crack o decode sa paraang na pagtutumbas ng isang letra sa isa pang letra. Ang lahat ng letra ay kabaliktaran lamang sa naunang letra at sunod nito ay katumbas lamang sa kasunod nito. Hal. A = B, B = C, W = X, at X = Y. Kaya masusing basahin ang tanong. Maaaring gumamit ng 3 beses na hint lamang sa kabuoan ng laro upang tulungan na malaman ang sagot. Kung handa na ay maaari ng pindutin ang “Simula na”.",
  videoUrl:
    "https://drive.google.com/file/d/1i9qr-cdvSTZHaQWkNzXsFSJgJsTHl-gd/preview",
  greetings:
    "Maligayang pagdating sa ating talakayan na pinamagatang “Parabula bilang isang Misteryosong Panitikan”. Ang paksang ito ay umiiral sa inyong pagkatuto sa parabula at kung papaano ang isang akda ay nakapagpapabago sa buhay sa pamamagitan ng pakikinig sa mensahe ng Diyos na iginawad sa pamamagitan ng karunungan ni Hesus. Halina’t alamin natin ito!",
  objective: `Ang mga sumusunod na layunin ay ninais na makamit sa sesyong ito, 
\nMost Essential Learning Competencies (MELCs): 
\nNapatutunayan ang mga pangyayari sa binasang parabula ay maaaring maganap sa tunay na buhay sa kasalukuyan (F9PB-IIIa-50 ).
\nLayuning Pangkabatiran: Nalalaman ang elemento ng parabula.
\nLayuning Pandamdamin: Napahahahalagahan ang mga pangyayari sa parabula na maaaring mangyari sa tunay na buhay.
\nLayuning Saykomotor: Nakatutuklas ng mga mahahalagang kaisipan gamit ang augustus code batay sa mga pangyayari sa parabula.`,
  rounds: [
    {
      imageUrl: "/three/1.png",
      instruction:
        "Mas magiging malinaw sa mga mangmang at tagapakinig ni Hesus kung ___________ ang dalawa o higit pang bagay.",
      hintText: "Ang dalawang bagay ay kinokompara sa isa't isa.",
      roundType: "word",
      answer: "pinaghahambing",
      title: "Antas 1",
      explanation:
        "Sa panahon ni Hesus marami ang mangmang at hindi gaano nakauunawa sa mga aral ng Diyos kung kaya ang paghahambing ng dalawa o higit pang magkakaibang tao, bagay, o lugar ay mas nagtitiyak ng malinaw at komprehensibong pagkakaunawa sa kuwento hindi lamang sa panlabas na anyo kundi pati na rin sa pag-uugaling pinapalooban nito. Nakapagbibigay din ito ng hinuha sa mga tagapakinig upang ikompara ang kanilang sarili sa mga pangyayari sa parabula.",
      expImage: "/explanation/three/1.png",
    },
    {
      imageUrl: "/three/2.png",
      instruction:
        "Hinuhubog ng parabula ang _________ at __________ na pagkatao ng isang tagapakinig.",
      hintText: "Pagbabago ng tao sa panloob at panlabas.",
      roundType: "word",
      answer: "moral at espiritwal",
      title: "Antas 2",
      explanation:
        "Daluyan ang parabula noong panahon ni Hesus maging ngayon sa kasalukuyan upang hubugin at baguhin ang pag-uugali ng isang tao sa mga pangyayaring nagaganap sa kaniyang buhay. Sa moral na aspekto tinutuwid nito ang baluktot na pag-uugali ng isang tao upang itama ang maling gawi nito habang sa espiritwal na aspekto ay paghubog sa paniniwala sa Poong Maykapal. Tinutuwid nito ang paniniwalang marami o walang Diyos na sa katunayan mayroong iisang Diyos na pinakamakapangyarihan sa lahat; ang simula at wakas.",
      expImage: "/explanation/three/2.png",
    },
    {
      imageUrl: "/three/3.png",
      instruction:
        "Ang parabula ay isang _____________ akda na kinakailangan suriin at unawain.",
      hintText: "Pagtatago ng mensahe",
      roundType: "word",
      answer: "PATALINGHAGA",
      title: "Antas 3",
      explanation:
        "Isa sa nagpapatingkad sa parabula ay pagtatago ng mensahe nito. Ang parabula ay balot ng misteryo na kailanman ay hindi ito madaling maunawaan sa lahat. Bakit hindi direkta itinuturo ni Hesus? Ang katotohanan minsan ay hindi maganda sa pandinig kaya ang pagtuturo ni Hesus ng ganitong paraan ay upang madaling maalala paraan ng pagpapaliwanag / pangaral ng Diyos. Maiksi lamang ito subalit kakikintalan ito sa mga tagapakinig sapagkat ito ay mayaman sa imahinasyon na kayang unawain ng mga tagapakinig.",
      expImage: "/explanation/three/3.png",
    },
    {
      imageUrl: "/three/4.png",
      instruction:
        "Ang parabula ay naglalaman ng _________________ kagaya ng ibang panitikan.",
      answer: "MAGANDANG KAISIPAN",
      hintText: "Ang pinakamahalagang ideya ng parabula.",
      roundType: "word",
      title: "Antas 4",
      explanation:
        "Ang magandang kaisipan o aral ang nagpapatingkad sa parabula sapagkat ninanais ni Hesus na mayroong matutuhan ang kaniyang mga tagapakinig sa mga pangyayaring nagaganap sa buhay ng isang tao. Sa aral na ibinigay ay nagdadala ito ng pagbabago sa kanilang mga nakaraang aksyon at pag-iisip sa kanilang buhay na kinakailangang maituwid upang ang kanilang buhay ay maliligtas sa kasalanan ng mundo.",
      expImage: "/explanation/three/4.png",
    },

    {
      imageUrl: "/three/5.png",
      instruction:
        "“Marahil, gawa sila mula sa iba’t ibang materyal at iba-iba rin ang kanilang kulay. May puti, may itim, may kulay tsokolate at may dilaw.” Ano ang inihahambing sa teksto sa tunay na buhay ng tao?",
      answer: "LIPI NG TAO",
      hintText: "Pilipino, Amerikano, European, Caucasian",
      roundType: "word",
      title: "Antas 5",
      explanation:
        "Inihahambing sa teksto ang pagkakaiba ng banga sa lipi o pangkat ng tao sa mundo. Ang mga puti ay tinutukoy nito ang mga magandang buhay, may itim at tsokalate ay ang mga nagtatrabahong bilad sa araw, ang mga dilaw ay ang mga nasa may kaya sa buhay. Inihahambing nito ang pagkakaiba ng tao batay sa ating kulay ng balat, gampanin sa lipunan, at ang pagkakilanlan sa lipunan.",
      expImage: "/explanation/three/5.png",
    },
    {
      imageUrl: "/three/6.png",
      instruction:
        "“Huwag mong kalilimutang ikaw ay isang bangang gawa sa lupa,” ang tagubilin  ng  Inang Banga  sa kaniyang anak.” Ano ang mensahe nito?",
      answer: "HUWAG KALILIMUTAN ANG PINAGMULAN",
      hintText: "Ang simula ng bagay-bagay sa buhay ng tao.",
      roundType: "word",
      title: "Antas 6",
      explanation:
        "Marami ang nawawala sa landas ng kanilang buhay at nakakalimutan ang kanilang pinagmulan dahil sa mga nangyayaring pagbabago sa buhay ng tao na nakapagdudulot ng kapahamakan sa kanilang sarili. Pinatutunayan lamang ng paalala ng inang banga sa ating buhay na kahit saan man mapadpad sa buhay ay huwag kalilimutan at talikuran ang pinagmulan natin sa ating buhay.",
      expImage: "/explanation/three/6.png",
    },
    {
      imageUrl: "/three/7.png",
      instruction:
        "“At ikaw ay nararapat na makisalamuha  lamang  sa ating  mga  kauri ng banga.” Ano ang ibig sabihin nito para sa kanyang anak na Banga?",
      answer: "HUWAG PAKIALAMAN ANG BUHAY NG IBA",
      hintText: "Anyo ng buhay ng tao.",
      roundType: "word",
      title: "Antas 7",
      explanation:
        "Magkakaiba-iba ang buhay ng tao sa lipunan, kagaya ng pagkakakaiba-iba ng mga banga sa parabula kaya huwag mong papakialaman ang kanilang buhay, sa halip ituon mo ito lamang sa iyong sarili.",
      expImage: "/explanation/three/7.png",
    },
    {
      imageUrl: "/three/8.png",
      instruction:
        "“Bakit  wala namang  masama  sa paliligo  sa  lawa kasamang  ibang  uring  banga.  Wala naman  kaming gagawing hindi tama,” bulong niya sa sarili. At sumunod siya sa porselanang banga at sinabing, “Oo, maliligo ako sa lawa  kasama mo.  Ngunit  saglit lamang,  nais  ko lang  na mapreskuhan.” Ano ang ibig sabihin ng sinambit ng Anak na Banga?",
      answer: "PAGPILI SA MGA PANIBAGONG KARANASAN",
      hintText: "Gustong malaman ang hindi pa nararanasan sa kaniyang buhay.",
      roundType: "word",
      title: "Antas 8",
      explanation:
        "Sa parabula, ang anak na banga ay hindi pa kailanman nakaliligo sa lawa subalit gusto niyang maranasan ang buhay ng ibang banga kahit na ito ay may kaakibat na panganib.",
      expImage: "/explanation/three/8.png",
    },
    {
      imageUrl: "/three/9.png",
      instruction:
        "“Habang siya’y  nabibitak at  unti-unting lumulubog sa ilalalim ng tubig, naalaala ng bangang lupa ang kaniyang ina.” Ano ang ibig sabihin ng Anak na Banga sa kaniyang pagkakalubog?",
      answer: "PAGSISI DAHIL SA PAGSUWAY",
      hintText:
        "Kahit kailanman ay ito hindi nauuna sa mga pangyayari sa buhay ng tao.",
      roundType: "word",
      title: "Antas 9",
      explanation:
        "Unti-unting lumubog ang anak na banga sa lawa dahil sa pagsuway nito sa utos ng kaniyang ina. Alam ng ating mga ina kung ano ang ikabubuti ng kanilang mga anak para sa kanila sapagkat alam na nila ang mangyayari rito. Ang inang banga bilang Poong Maykapal ay nagbibigay ng paalala sa ating buhay upang hindi tayo mapahamak subalit tayo bilang anak na banga o anak ng Diyos ay minsang sumusuway na nagdudulot ng kapahamakan na minsan ito ang magdadala ng aral at minsan kamatayan sa hindi pagsunod nito na siyang pagsisi sa huli.",
      expImage: "/explanation/three/9.png",
    },
    {
      imageUrl: "/three/10.png",
      instruction:
        "Batay sa parabulang “Mensahe ng Butil ng Kape”, sa paanong paraan inihahambing ng ama ang kalagayan ng isang tao kapag nahaharap sa pagsubok?",
      answer: "PAGLULUTO",
      hintText: "Ang pangunahing ginagawa sa kusina.",
      roundType: "word",
      title: "Antas 10",
      explanation:
        "Sa paraaang pagluluto sapagkat sila ay magsasaka at ito ang kanilang kabuhayan. Ginamit ang mga pagkain sa paligid upang ipakita ang mensahe ng pagsubok sa buhay.",
      expImage: "/explanation/three/10.png",
    },
    {
      imageUrl: "/three/11.png",
      instruction:
        "Mula sa parabulang, Mensahe ng Butil ng Kape”, ano ang sinisimbolo ng carrot, itlog, at butil ng kape sa buhay ng tao?",
      answer: "PAGHARAP SA PAGSUBOK",
      hintText: "Kung mayroong problema ay mayroon ding solusyon.",
      roundType: "word",
      title: "Antas 11",
      explanation:
        "Ang carrot, itlog, at butil ng kape ay sumisimbolo sa iba't ibang paraan ng pagharap sa mga pagsubok. Ang carrot ay nanghihina, ang itlog ay tumitigas, at ang butil ng kape ay nagiging mas mabuti.",
      expImage: "/explanation/three/11.png",
    },
    {
      imageUrl: "/three/12.png",
      instruction:
        "Bakit kinakailangan na alamin ang nakatagong mensahe sa parabula?",
      answer:
        "TANGKILIKIN ANG MENSAHE NG DIYOS UPANG MAPATIBAY ANG ISPIRITWAL NA PANINIWALA AT MAPAUNLAD ANG KRITIKAL NA PAG-IISIP SA BUHAY",
      hintText: undefined,
      roundType: "word",
      title: "Konklusyon",
      explanation:
        "Ang pagtangkilik sa mensahe ng Diyos ay nagdadala ng karunungan sa buhay ng tao, nagpapalawak sa kritikal na pag-iisip, at nagbibigay ng lakas sa ispiritwal na aspeto ng pamumuhay.",
      expImage: "/explanation/three/12.png",
    },
  ],
  ins: [
    "/instructions/three/1.png",
    "/instructions/three/2.png",
    "/instructions/three/3.png",
    "/instructions/three/4.png",
    "/instructions/three/5.png",
    "/instructions/three/6.png",
  ],
};

export default breakoutThree;
